...
function run_spec(dirname, options) {
  ...
  fs.readdirSync(dirname).forEach((filename) => {
    ...
    if (path.extname(filename) !== '.snap' && fs.lstatSync(filepath).isFile() && filename[0] !== '.' && filename !== 'jsfmt.spec.js') {
      let rangeStart = 0;
...
function prettyprint(src, options) {
  ...
  if (options.cursorOffset >= 0) {
    result.formatted = result.formatted.slice(0, result.cursorOffset) + '<|>' + result.formatted.slice(result.cursorOffset);
  }
